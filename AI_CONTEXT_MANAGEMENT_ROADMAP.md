...
### 🎯 목적
- 프리셋 추천이 반복되는 문제를 줄이고 사용자 경험을 다양화
- JSON 파싱 실패 시에도 결과가 단조로워지지 않도록 폴백 경로 다양성 보장
- 기존 시스템 프롬프트·중앙 경로를 재사용(SSOT/DRY)하여 성능·안정성 유지

### ✨ 주요 변경사항
1) 시스템 프롬프트 다양성 지시 추가(재사용 기반)
- 경로: UnifiedAIServiceImpl.generateOptimizedSystemPrompt(for:model:) → presetRecommendation 분기
- 내용: 이름·조합·표현의 반복 회피, 시간대/감정/환경 맥락 반영, 시적인 한국어 제목을 매번 새롭게 구성하도록 “다양성 지시”를 명시
- 기존 JSON 출력 강제 규칙(스키마 유도)은 그대로 유지 → 파싱 안정성 영향 없음

2) 폴백 후보 선택의 랜덤 보장
- 경로: AIResponseParser.chooseBasicCandidate(for:)
- 동작: 입력 키워드로 후보군을 필터링한 뒤, 무조건 랜덤(randomElement)으로 최종 선택
- 효과: JSON 실패 또는 비JSON 휴리스틱 경로에서도 추천 다양성 확보

### 🚫 비적용 결정(명시)
- “최근 10개 이름” 중복 회피: 미적용(요청 정책)
- 외부 Naver seed 랜덤화: 외부 클라우드 모델 제거 예정이므로 미적용

### 🧪 검증 시나리오
- 동일 컨텍스트에서 “대나무숲 분석 추천받기” 5회 반복 시:
  - JSON 성공 경로: 프리셋명/사유 표현이 반복 없이 변주되는지 확인
  - JSON 실패(휴리스틱/기본) 경로: 매회 다른 후보가 랜덤으로 선택되는지 확인
  - 채팅 버블에서 사유 섹션(간단한 이유/감정 패턴과 상황과의 연결성) 멀티라인 정상 표시

### 🔍 변경 파일 요약
- UnifiedAIServiceImpl.swift
  - generateOptimizedSystemPrompt(for:model:) 내 presetRecommendation 규칙에 “다양성 지시” 추가
- AIResponseParser.swift
  - chooseBasicCandidate(for:)에서 후보군 필터링 후 확실한 랜덤 선택으로 변경

### 🎓 설계 원칙
- DRY/SSOT: 기존 시스템 프롬프트 생성 경로 재사용, 규칙만 보강
- KISS: 파서·UI·세션 로직 변경 최소화
- 성능/TTI/메모리 영향 최소화 유지
