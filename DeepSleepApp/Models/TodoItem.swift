...
extension TodoItem {
    ...
    /// 자동 완료 기준 시각(Single source of truth)
    /// - 하루종일(빠른등록): nil을 반환하여 자동 완료에서 제외
    /// - 그 외: endDate가 유효하면 endDate, 없으면 dueDate
    var autoCompletionDeadline: Date? {
        if isAllDayQuickRegistration { return nil }
        if let end = endDate, end > dueDate { return end }
        return dueDate
    }

    /// 현재 시각 기준 자동 완료 여부 판단
    /// - Parameter now: 비교 기준 시각(기본값: 현재)
    /// - Returns: 자동 완료해야 하면 true
    func shouldAutoComplete(at now: Date = Date()) -> Bool {
        guard !isCompleted else { return false }
        guard let deadline = autoCompletionDeadline else { return false }
        return deadline <= now
    }
}
