...
public enum InvalidationReason: String, Codable {
    ...
    @available(
        *, deprecated,
        message:
            "이전 통합 사유. 이제 .expired / .personaChanged / modelSelectionChanged / modeChanged / toneChanged 세분화 사용."
    )
    case expiredOrPersonaChanged  // 레거시: 새 코드에서는 사용 금지 (검색/로그 잔재 정리 예정)
    case none  // (주로 Hit 로깅에 사용)
}

public enum ContextCacheEvent: String, Codable {
    case hit
    case miss
}
...
public struct CacheLogEntry: Codable {
    ...
    public let event: ContextCacheEvent
    public let reason: InvalidationReason
    public let ageSeconds: Int?
    public let caller: String?

    public init(
        event: ContextCacheEvent, reason: InvalidationReason, age: TimeInterval?, caller: String?
    ) {
        self.timestamp = Date()
        self.event = event
        self.reason = reason
