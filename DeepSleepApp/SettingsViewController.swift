...
class SettingsViewController: UIViewController {
    ...
    private func loadCurrentSettings() {
        // UserDefaults에서 현재 설정 불러오기
        if let modelRaw = UserDefaults.standard.string(forKey: "selectedAIModel"),
           let model = AIModelType(rawValue: modelRaw) {
...
extension SettingsViewController {
    ...
    private func showAIModelSelection() {
        // ✅ AI 모델 선택 기능 활성화 - SwiftUI 뷰 연결
        let aiModelSettingsView = AIModelSettingsView()
        let hostingController = UIHostingController(rootView: aiModelSettingsView)
        hostingController.title = "대나무숲 친구 선택"
        navigationController?.pushViewController(hostingController, animated: true)
    }
    
    private func showModelCharacteristics() {
        let characteristicsVC = ModelCharacteristicsViewController()
