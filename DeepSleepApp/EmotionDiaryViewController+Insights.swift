...
extension EmotionDiaryViewController {
    ...
    func updateInsightView() {
        ...
        guard !diaryEntries.isEmpty else {
            ...
            spacerView.setContentHuggingPriority(.init(1), for: .vertical)
            spacerView.setContentCompressionResistancePriority(.init(1), for: .vertical)
        ...
        // 6. ⭐️ 중요한 일정 (드롭다운)
        let importantCount = getImportantScheduleData().count
        let importantCard = createInsightCard(
            title: "⭐️ 중요한 일정",
            content: "\(importantCount)개 선정",
            color: .systemPink.withAlphaComponent(0.1),
            isDropdownEnabled: true,
        ...
        // 7. 🔥 연속 기록 (드롭다운)
        let streak = calculateDiaryStreakDays()
        let streakCard = createInsightCard(
            title: "🔥 연속 기록",
            content: "\(streak)일 연속",
        ...
        insightStackView.addArrangedSubview(streakCard)

        // 8. 🕒 시간대 패턴 (드롭다운)
        let timeStats = analyzeTimeOfDay()
        let timeOfDayCard = createInsightCard(
            title: "🕒 시간대 패턴",
            content: "가장 많이 기록: \(timeStats.top)",
            color: .systemIndigo.withAlphaComponent(0.1),
            isDropdownEnabled: true,
            dropdownType: .timeOfDayPattern
        )
        insightStackView.addArrangedSubview(timeOfDayCard)

        // 9. 🗓️ 월간 감정 요약 (드롭다운)
        let monthStats = analyzeMonthlyEmotions()
        let monthSummaryText = monthStats.top != nil ? "\(monthStats.top!) (\(monthStats.topPercent)%)" : "데이터 없음"
        let monthlyEmotionCard = createInsightCard(
            title: "🗓️ 월간 감정 요약",
            content: monthSummaryText,
            color: .systemYellow.withAlphaComponent(0.1),
            isDropdownEnabled: true,
            dropdownType: .monthlyEmotionSummary
        )
        insightStackView.addArrangedSubview(monthlyEmotionCard)

        // 10. 🔎 키워드 요약 (드롭다운)
        let keywords = extractTopKeywords(limit: 3)
        let keywordLine = keywords.isEmpty ? "키워드 없음" : keywords.joined(separator: ", ")
        let keywordCard = createInsightCard(
            title: "🔎 키워드 요약",
            content: keywordLine,
            color: .systemGray6,
            isDropdownEnabled: true,
            dropdownType: .keywordSummary
        )
        insightStackView.addArrangedSubview(keywordCard)
    ...
    enum InsightDropdownType: Int {
        ...
        case streak = 7
        case timeOfDayPattern = 8
        case monthlyEmotionSummary = 9
        case keywordSummary = 10
    }
    ...
    private func getDropdownContent(for type: InsightDropdownType) -> String {
        ...
        case .streak:
            return generateStreakDetail()
        case .timeOfDayPattern:
            return generateTimeOfDayPatternDetail()
        case .monthlyEmotionSummary:
            return generateMonthlyEmotionSummaryDetail()
        case .keywordSummary:
            return generateKeywordSummaryDetail()
        }
    }
    ...
    // MARK: - NEW: Enhanced Insights Generators

    private func generateStreakDetail() -> String {
        let streak = calculateDiaryStreakDays()
        var detail = "연속 기록: \(streak)일\n"
        ...
        for i in 0..<7 {
            if let day = calendar.date(byAdding: .day, value: -i, to: today) {
                let count = diaryEntries.filter {
                    calendar.isDate($0.date, inSameDayAs: day)
                }.count
                let mark = count > 0 ? "✅" : "—"
                lines.append("• \(formatter.string(from: day)) \(mark) \(count)개")
            }
        }
    ...
    private func generateTimeOfDayPatternDetail() -> String {
        let stats = analyzeTimeOfDay()
        var detail = "시간대별 기록 빈도:\n"
        let order = ["새벽", "아침", "오후", "밤"]
        for key in order {
            let c = stats.counts[key] ?? 0
            detail += "• \(key): \(c)회\n"
        }
        return detail
    }

    private func generateMonthlyEmotionSummaryDetail() -> String {
        let m = analyzeMonthlyEmotions()
        guard m.total > 0 else { return "이번 달 기록이 없습니다." }
        var detail = "이번 달 감정 분포:\n"
        for (emotion, cnt) in m.counts.prefix(5) {
            let pct = Int((Double(cnt) / Double(m.total)) * 100.0)
            detail += "• \(emotion): \(cnt)회 (\(pct)%)\n"
        }
        return detail
    }

    private func generateKeywordSummaryDetail() -> String {
        let top = extractTopKeywords(limit: 10)
        if top.isEmpty { return "키워드 추출 결과가 없습니다." }
        var detail = "자주 등장한 키워드 Top 10:\n"
        for (idx, word) in top.enumerated() {
            detail += "\(idx+1). \(word)\n"
        }
        return detail
    }

    // MARK: - NEW: Analysis Helpers (DRY)

    private func calculateDiaryStreakDays() -> Int {
        let calendar = Calendar.current
        let dayKeyFormatter = DateFormatter()
        dayKeyFormatter.dateFormat = "yyyy-MM-dd"

        let daySet: Set<String> = Set(diaryEntries.map { dayKeyFormatter.string(from: calendar.startOfDay(for: $0.date)) })
        var streak = 0
        var cursor = calendar.startOfDay(for: Date())

        while daySet.contains(dayKeyFormatter.string(from: cursor)) {
            streak += 1
    ...
    private func analyzeTimeOfDay() -> (top: String, counts: [String: Int]) {
        var counts: [String: Int] = ["새벽": 0, "아침": 0, "오후": 0, "밤": 0]
        let calendar = Calendar.current
        for e in diaryEntries {
            ...
            case 0...5: bucket = "새벽"
            case 6...11: bucket = "아침"
            case 12...17: bucket = "오후"
            default: bucket = "밤"
            }
            counts[bucket, default: 0] += 1
        }
        let top = counts.max(by: { $0.value < $1.value })?.key ?? "데이터 없음"
        return (top, counts)
    }

    private func analyzeMonthlyEmotions() -> (top: String?, topPercent: Int, total: Int, counts: [(String, Int)]) {
        let calendar = Calendar.current
        let now = Date()
        let m = calendar.component(.month, from: now)
        let y = calendar.component(.year, from: now)

        let monthEntries = diaryEntries.filter {
            calendar.component(.month, from: $0.date) == m && calendar.component(.year, from: $0.date) == y
        }
        guard !monthEntries.isEmpty else { return (nil, 0, 0, []) }

        var map: [String: Int] = [:]
        for e in monthEntries { map[e.selectedEmotion, default: 0] += 1 }
        let sorted = map.sorted { $0.value > $1.value }
        let top = sorted.first?.key
        let total = monthEntries.count
        let topPct = sorted.first.map { Int((Double($0.value) / Double(total)) * 100.0) } ?? 0
        return (top, topPct, total, sorted.map { ($0.key, $0.value) })
    }

    private func extractTopKeywords(limit: Int = 5) -> [String] {
        // 간단한 불용어 목록 (KISS, YAGNI 기준 최소 구성)
        let stop: Set<String> = [
            "그리고","하지만","그래서","또","너무","정말","그냥","오늘","어제","내일",
            "나는","내가","내게","우리는","너는","그","이","저","것","거","좀","더","잘","못",
            "입니다","합니다","했다","했다가","하는","하고","하게","해서","해서요","인데","인데요",
            "요","은","는","이","가","을","를","에","에서","으로","로","와","과","도","만","까지",
            "랑","께","처럼","이라","라고","하며","다가","또는","혹은"
        ]

        var freq: [String: Int] = [:]
        let letters = CharacterSet.letters
        for entry in diaryEntries {
            let rawTokens = entry.userMessage
                .components(separatedBy: .whitespacesAndNewlines)
                .map { token -> String in
                    let filtered = String(token.unicodeScalars.filter { letters.contains($0) })
