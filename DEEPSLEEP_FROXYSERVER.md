...
## 중요: 온디바이스 경로와 프록시 분리
- 온디바이스 경로는 프록시 서버를 경유하지 않는다. 클라이언트(App)에서 llama.cpp 바인딩/세션을 직접 사용한다.
- 서버 경유 캐시(예: 공급자 캐시, cachedContents)와 온디바이스 KV 접두부 캐시(세션 상태/KV) 개념을 명확히 분리한다.
  - 서버 캐시: 공급자/프록시 영역에서 응답 JSON/프리픽스를 재사용(헤더: X-Cache-*, X-Canary 등)
  - 온디바이스 KV: 단말 llama 세션의 시스템 프롬프트 접두부만 저장/복원하여 디코딩을 생략([KVCache] 로그로 식별)

## 참고: 서버 모드 병행 시
- llama.cpp 서버 모드 병행 시에만 적용되는 팁을 이 섹션으로 이동:
