...
- ✅ **AppConfig.swift**: 보안 강화된 Bundle.main.object 사용
...
## 2025-09-01 운영 동기화: 모델별 캐싱 시스템

요약
- 현재 코드/배포 상태 기준으로 본 문서를 최신화했습니다. 서버는 공급자별 캐싱(MVP)이 적용되어 프로덕션 배포 완료, iOS는 무효화 헤더 연동 및 캐시 헤더 파싱이 반영됨.

아키텍처 결정
- 듀얼 레이어: 앱 3시간 캐시(UX) + 서버 공급자 캐싱(비용). 서버가 SSoT로 소유권/무효화/메트릭 관리
- KV 키 스키마: cache:{provider}:{model}:{personaKey} (+ prefix metrics for OpenAI)
- 무효화 흐름: iOS 이벤트→X-Context-Invalidation 헤더 1회성 전송→서버 KV 삭제→다음 호출 write

서버 구현(Cloudflare Workers: `emozleep/worker.js`)
- /v1/chat: providerCaching 파싱(enable/strategy/ttlSeconds/cacheKey), 무효화 헤더 처리
- CORS: Allow-Headers에 X-Context-Invalidation 추가, Expose-Headers에 X-Cache-* 추가
- 응답 헤더: X-Cache-Provider/Action/TTL/Tokens
- Anthropic: cache_control(ephemeral, ttl=3600s), 30분 경과 시 write 강제
- Gemini: caches.create(ttl=3600s) + 매 요청 PATCH(updateMask=ttl)로 ttl=3600s 갱신하여 최대 3시간 운용
- OpenAI: 시스템 프리픽스 해시/길이 KV 기록(안정성 모니터링)
- Naver: 캐싱 미지원 처리

클라이언트(iOS)
- `AIContextManager.swift`: clearCache(reason:) → pendingInvalidationReason 저장, consumeInvalidationReasonForHeader() 제공
- `UnifiedAIServiceImpl.swift`: providerCaching 기본 전송(enable=true, strategy=auto, ttlSeconds=3600), X-Cache-* 헤더 파싱 후 메타데이터에 저장, X-Context-Invalidation 1회성 헤더 전송

운영 상태
- 프로덕션 URL: https://emozleep-production.vinny4920-081.workers.dev
- 최근 배포 Version ID: c18d6a9c-f4ad-43ed-9689-dce0210e60b5
- 로그 관찰 지표: provider별 write/read 비율, Gemini PATCH 200 비율, OpenAI 프리픽스 해시 변동

다음 단계
- /v1/metrics: hit_rate, cache_write/read_tokens, est_savings_usd 집계 및 대시보드 연동
- NAVER 키 단일화(NAVER_CLOUD_API_KEY) 완전 이관
- iOS 린트/빌드 경고 재확인 및 정리
